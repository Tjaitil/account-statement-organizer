...
export type EntryTransactionFormat<T extends string = string> = Record<
  T,
  string
>;

export type EntryTransactionFormatOne = EntryTransactionFormat<
  'Dato' | 'Type' | 'Beløp' | 'Beskrivelse'
>;
export type EntryFormatOne = StatementFormat<EntryTransactionFormatOne>;

export type EntryTransactionFormatTwo = EntryTransactionFormat<
  | 'Utført dato'
  | 'Bokført dato'
  | 'Rentedato'
  | 'Beskrivelse'
...
export type StatementFormats =
  | EntryFormatOne
  | EntryFormatTwo
  | EntryFormatUnknown;
export type EntryFormat = EntryFormatOne | EntryFormatTwo | EntryFormatUnknown;

export function isEntryFormatOne(
  format: EntryFormat['columns'],
): format is EntryFormatOne['columns'] {
  return Array.isArray(format) && format.length > 0 && 'Beløp' in format[0];
}

export function isEntryFormatTwo(
  format: EntryFormat['columns'],
): format is EntryFormatTwo['columns'] {
  return (
    Array.isArray(format) && format.length > 0 && 'Utført dato' in format[0]
  );
}

export function isTransactionFormatOne(
  format:
    | EntryTransactionFormatOne
    | EntryTransactionFormatTwo
    | StatementFormatUnknown,
): format is EntryTransactionFormatOne {
  return 'Dato' in format;
}

export function isTransactionFormatTwo(
  format:
    | EntryTransactionFormatOne
    | EntryTransactionFormatTwo
    | StatementFormatUnknown,
): format is EntryTransactionFormatTwo {
