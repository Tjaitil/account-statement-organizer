<template>
  <div>
    <p class="mb-4">
      {{ t('Hang tight while we categorize your transactions') }}
    </p>
    <PrimeProgressBar :value="progress" display-value-template="{value}%" />
  </div>
</template>
...
const organize = () => {
  props.uncategorizedTransactions.forEach(uncTransaction => {
    if (uncTransaction.amount > 0) {
      const category = findCategory('income');
      if (category) {
        category.amount += uncTransaction.amount;
        const categorisedTransaction = createTransaction(uncTransaction, {
          category: 'income',
        });
    ...
    for (const map of transactionMap) {
      const match = map.patterns.some(map =>
        uncTransaction.description.toLowerCase().includes(map),
      );

      if (match) {
        const category = findCategory(map.category);
        if (category) {
          category.amount += uncTransaction.amount;
          const categorisedTransaction = createTransaction(uncTransaction, map);
          category.transactions.push(categorisedTransaction);
          isCategorySet = true;
          break;
        }
    ...
    if (isCategorySet === false) {
      const mappedTransaction = createTransaction(uncTransaction, {
        category: 'other',
      });
      const category = findCategory('other');
      if (category) {
        category.amount += uncTransaction.amount;
...
const findCategory = (
  description: TransactionGroupName,
  ...
  for (const element of organizedStatement.categories) {
    ...
    if (element.categories) {
      const subElement = element.categories.find(
        subElement => subElement.name === description,
      );
      if (subElement) {
        return subElement;
      }
