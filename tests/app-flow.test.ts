...
test('upload file and press submit', async ({ page }) => {
  ...
  page.on('filechooser', async fileChooser => {
    await fileChooser.setFiles(path.join(dirname, '../src/assets/demo-file.csv'));
  });

  await page.getByText('Velg').click();

  await page.getByRole('button', { name: 'Organiser', }).click({timeout: 10000});

  await expect(page.getByText('Vent mens vi kategoriserer transaksjonene dine')).toBeVisible();
